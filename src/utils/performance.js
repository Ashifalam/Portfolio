  ...
  if (typeof window !== 'undefined' && 'performance' in window) {
    ...
    Object.keys(metrics).forEach((metric) => {
      if (metrics[metric] > budgets[metric]) {
        console.warn(`Performance budget exceeded for ${metric}: ${metrics[metric]}ms (budget: ${budgets[metric]}ms)`);
      } else {
        console.log(`✅ ${metric}: ${metrics[metric]}ms (within budget: ${budgets[metric]}ms)`);
      }
    });
